...
| **Acceptance**     | Small ten-second answer ring; balanced cumulative score indicator inside the wizard arena; content-driven casting/recap stage contains every control; compact five-second recap retains both submitted guesses; both players can skip together with a plain inline readiness count; three-second countdown unchanged; full per-round details remain accessible in end-summary disclosure. |
